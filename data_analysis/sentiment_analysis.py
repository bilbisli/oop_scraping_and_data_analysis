...
def analyse_sentiment(article, model=None):
    if model is None:
...
def get_model():
    ...
    return pipeline("sentiment-analysis", model=model_name, tokenizer=model_name, max_length=512)
...
What's a state funeral? Will shops close? And other questions"""
    print(analyse_sentiment(article))